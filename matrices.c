...
void op1()
{
    ...
    if(x==1)
    {
        ...
        for(i=0; i<y; i++)
        {
            printf("Insert the value fo the %d coordinate of the vector: ", i+1);
            scanf("%f", &a);
...
void oppos()
{
    ...
    printf("\nInsert a name for the inverse matrix of %s: ", str1);
    read_text(str2, sizeof(str2), 1);
    fp1 = fopen(str2, "wb");
    fwrite(&r1, sizeof(int), 1, fp1);
    fwrite(&c1, sizeof(int), 1, fp1);
    ...
    printf("\nThe inverse matrix of %s is the following:\n", str1);
    for(i=0; i<r1; i++)
    {
        printf("|");
        for(j=0; j<c1; j++)
